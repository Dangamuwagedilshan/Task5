...
public class Experiment12 {
	...
	class eRemover implements Function<String, String>{
		...
		public eRemover(String club) {
			// TODO Auto-generated constructor stub
		}

		public String apply(String ClbName) {
			return ClbName.replace("es", "x");
		}
	}
	...
	public void run() {
		...
		table.stream().map(n-> new eRemover( n.getClub())).forEach(ClbName->System.out.println(ClbName));
		
		
		//table.stream().map(new eRemover()).forEach(clbName-> System.out.println(clbName.getClub()));
